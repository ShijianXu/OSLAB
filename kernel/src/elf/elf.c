...
//	readsect(&direct, ELF_OFFSET_IN_DISK + sizeof(bitmap));
/*	int index = 0;
	for(int i = 0; i < NR_DIR_ENTRY; i ++)
	{
		if(direct.entry[i].inode_offset != -1)
		{
//			printk("HERE\n");
			printk("%s\n", direct.entry[i].filename);
			index = i;
			break;
		}
	}

	struct act_inode node;
	int offset = direct.entry[index].inode_offset;
	readseg((uint8_t *)&node.data, 512, ELF_OFFSET_IN_DISK + offset * 512);
	for(int i = 0; i < NR_INODE_ENTRY; i ++)
	{
		if((int)node.data.data_block_offset[i] > 0)
			printk("%d\n",node.data.data_block_offset[i]);
	}
*/
	int pcb_index = pcb_alloc(); //分配的pcb在表中的位置    
	int pid = pid_alloc();       //分配pid

printk("game's pcb_index = %d 	game's pid = %d\n",pcb_index, pid);
	pcb_new(pid, 0, pcb_index);  //做一些初始化工作，状态为new
...
//	readseg((uint8_t*)buffer, 4096, ELF_OFFSET_IN_DISK);
	...
	/*	for(int i = 0; i < NR_INODE_ENTRY; i ++)
	{
		if((int)inode.data_block_offset[i] > 0)
